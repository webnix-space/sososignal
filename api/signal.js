...
async function fetchLiveData() {
  try {
    ...
    const [currenciesRes, etfRes, ssiRes] = await Promise.all([
      fetchWithTimeout(`${SOSO_BASE}/currency/list`, { headers }, 5000),
      fetchWithTimeout(`${SOSO_BASE}/etf/summary`, { headers }, 5000),
      fetchWithTimeout(`${SOSO_BASE}/indices/ssiTop7/market-snapshot`, { headers }, 5000)
    ]);

    const currenciesJson = await currenciesRes.json();
    const etfJson = await etfRes.json();
    const ssiJson = await ssiRes.json();

    const currencies = currenciesJson.data || [];

    const getSnap = async (sym) => {
      const c = currencies.find(x => x.symbol === sym);
      if (!c) return null;
      const r = await fetchWithTimeout(
        `${SOSO_BASE}/currency/${c.id}/market-snapshot`,
        { headers }, 5000
      );
    ...
    const [btc, eth, sol] = await Promise.all([
      getSnap('BTC'), getSnap('ETH'), getSnap('SOL')
    ]);

    const etfData = etfJson.data || {};
    const ssiData = ssiJson.data || {};
    ...
    return {
      ...
      etf: {
        totalNet: etfData.total_net_inflow || 0,
        btcTotal: etfData.total_asset || 0
      },
      ssi: {
        top7Price: ssiData.price || 0,
        top7Ch: ssiData.price_change_24h || 0
      },
...
Respond ONLY with valid JSON (no markdown, no explanation outside JSON):
{
  ...
  "data_sources": [
    ...
    {"name": "ETF Flow", "status": "${liveData?.etf ? 'Live' : 'Unavailable'}"},
    {"name": "SSI Top7 Index", "status": "${liveData?.ssi ? 'Live' : 'Unavailable'}"}
  ],
