// api/signal.js - Fixed: Inject live data into ALL AI prompts
  ...
  try {
    ...
    if (mode === 'chat' || mode === 'signal') {
      try {
        ...
        // Fetch prices
        const pricesRes = await fetch(`${SOSO_BASE}/currency/list`, { headers, signal: AbortSignal.timeout(5000) });
        ...
        const getPrice = async (sym) => {
          ...
          if (!c) return null;
          const r = await fetch(`${SOSO_BASE}/currency/${c.id}/market-snapshot`, { headers, signal: AbortSignal.timeout(5000) });
        ...
        // Fetch ETF
        const etfRes = await fetch(`${SOSO_BASE}/etf/summary`, { headers, signal: AbortSignal.timeout(5000) });
