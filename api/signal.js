...
async function fetchLiveData() {
  try {
    ...
    // Try the shared cache soso.js already populates on its poll cycle first
    // — this is what stops "Load Signals" from firing its own independent
    // burst of ~12 SoSoValue calls every click.
    ...
    const currencyIds = cachedPrices ? null : await getCurrencyIdMap(headers);
    ...
    const getCurrencySnap = async (sym) => {
      ...
      if (!id) { console.error(`[signal] no currency_id resolved for ${sym}`); return null; }
      const r = await fetchWithTimeout(`${SOSO_BASE}/currencies/${id}/market-snapshot`, { headers }, 5000);
      if (!r.ok) {
        const body = await r.text().catch(() => '');
        console.error(`[signal] ${sym} market-snapshot failed: HTTP ${r.status} — ${body.slice(0, 200)}`);
        return null;
      }
      const j = await r.json();
    ...
    const getEtfSummary = async () => {
      ...
      if (!r.ok) {
        const body = await r.text().catch(() => '');
        console.error(`[signal] etfs/summary-history failed: HTTP ${r.status} — ${body.slice(0, 200)}`);
        return null;
      }
      const j = await r.json();
      const list = Array.isArray(j) ? j : (j.data || []);
      if (!Array.isArray(list) || !list.length) {
        console.error(`[signal] etfs/summary-history returned no usable list — ${JSON.stringify(j).slice(0, 200)}`);
        return null;
      }
      return list[list.length - 1]; // most recent entry
    };
    ...
    const getSsiSnap = async () => {
      if (cachedSector && cachedSector.length) {
        ...
        // rather than firing a separate live call for a close substitute.
      ...
      if (!ticker) { console.error('[signal] no SSI Top7 ticker resolved from /indices'); return null; }
      const r = await fetchWithTimeout(`${SOSO_BASE}/indices/${ticker}/market-snapshot`, { headers }, 5000);
      if (!r.ok) {
        const body = await r.text().catch(() => '');
        console.error(`[signal] indices/${ticker}/market-snapshot failed: HTTP ${r.status} — ${body.slice(0, 200)}`);
        return null;
      }
      const j = await r.json();
      return j.data || j;
