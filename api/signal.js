...
// + Audit Trail + Confidence Fix

import { Redis } from '@upstash/redis';

const GROQ = 'https://api.groq.com/openai/v1/chat/completions';

// In-memory audit log (use Redis/DB in production)
const auditLog = [];
...
export default async function handler(req, res) {
  ...
  if (!KEY) {
    logAudit('auth_check', {}, 'error', 'GROQ_API_KEY not set');
  ...
  if (mode === 'chat' || mode === 'analyze') {
    logAudit('chat_request', { mode, asset });
  ...
  // ── SIGNAL MODE ─────────────────────────────────────────────────────────
  logAudit('signal_request', { asset, hasPrompt: !!prompt });
...
Generate a trading signal. Respond ONLY with this JSON:
{
  ...
  if (!model1Result) {
    logAudit('signal_failed', {}, 'error', 'Signal generation failed');
...
Respond ONLY with JSON: {"agree":true,"adjusted_confidence":68,"risk_flags":["specific risk 1","specific risk 2"],"contrarian":"strongest counter-argument","final_recommendation":"CONFIRM"}`;
  ...
  // FIXED: Use confidence_evolution final running_total as base confidence
  let baseConfidence = model1Result.confidence || 50;
  if (model1Result.confidence_evolution && model1Result.confidence_evolution.length > 0) {
    const lastStep = model1Result.confidence_evolution[model1Result.confidence_evolution.length - 1];
    if (lastStep?.running_total) {
      baseConfidence = lastStep.running_total;
    }
  }

  let finalConf = baseConfidence;
  if (riskCheck?.adjusted_confidence) {
    finalConf = Math.round(baseConfidence * 0.6 + riskCheck.adjusted_confidence * 0.4);
  }
  ...
  // Ensure confidence matches the evolution
  if (model1Result.confidence_evolution && model1Result.confidence_evolution.length > 0) {
    model1Result.confidence_evolution[model1Result.confidence_evolution.length - 1].running_total = finalConf;
  }
