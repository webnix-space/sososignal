...
// FIXED: Validates price data exists before generating signals. Falls back to SoDEX prices.
...
// FIXED: Fetch live prices from SoDEX API as primary source
async function fetchLivePrices() {
  const prices = {};
  try {
    // Try SoDEX first (proven working in screenshots)
    const sodex = await fetch('https://mainnet-gw.sodex.dev/api/v1/spot/markets/tickers', {
      headers: { 'Accept': 'application/json' },
      signal: AbortSignal.timeout(5000)
    });
    if (sodex.ok) {
      const raw = await sodex.json();
      const items = raw?.data || [];
      for (const t of items) {
        const sym = (t.symbol || '').replace(/^v/, '').replace(/_vUSDC$/, '').replace(/_v/, '/');
        if (sym && t.lastPx) {
          const base = sym.split('/')[0];
          if (['BTC','ETH','SOL','BNB'].includes(base)) {
            prices[base] = {
              price: parseFloat(t.lastPx),
              change24h: parseFloat(t.changePct || 0) * 100
            };
          }
        }
      }
    }
  } catch (e) { console.error('SoDEX price fetch failed:', e.message); }

  // If SoDEX failed, try CoinGecko
      ...
      if (cg.ok) {
        ...
        for (const [id, sym] of Object.entries(map)) {
          if (d[id]?.usd) {
            prices[sym] = {
              price: d[id].usd,
              change24h: (d[id].usd_24h_change || 0) * 100
            };
          }
        }
      }
    } catch (e) { console.error('CoinGecko price fetch failed:', e.message); }
  }
...
export default async function handler(req, res) {
  ...
  // FIXED: Fetch live prices before generating signals
  const livePrices = await fetchLivePrices();
  const priceContext = Object.entries(livePrices).map(([sym, d]) => 
    `${sym}: $${d.price.toFixed(2)} (${d.change24h >= 0 ? '+' : ''}${d.change24h.toFixed(2)}% 24h)`
  ).join(', ');

  // If no prices available, return clear error instead of failing silently
  if (Object.keys(livePrices).length === 0) {
    return res.status(503).json({
      ok: false,
      error: 'No live price data available for signal generation. Price sources may be rate-limited. Check System Logs.',
      livePrices: false
    });
  }
  ...
  if (mode === 'chat' || mode === 'analyze') {
    ...
    for (const model of models) {
      try {
        ...
        return res.status(200).json({ result, model, prices: livePrices });
      } catch (e) {
...
Respond ONLY with JSON: {"agree":true,"adjusted_confidence":68,"risk_flags":["specific risk 1","specific risk 2"],"contrarian":"strongest counter-argument","final_recommendation":"CONFIRM"}`;
  ...
  const finalResult = {
    ...
    prices: livePrices // FIXED: Include live prices in response
  };
  ...
  try {
    ...
    if (redis) {
      const signalRecord = JSON.stringify({
        ...
        confidence: finalConf,
        confidence_evolution: model1Result.confidence_evolution || [],
        entryPrice: price || livePrices[asset || 'BTC']?.price || 0,
        etfFlow: etfFlow || 0,
