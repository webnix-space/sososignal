...
// FIXED: Redis env var (KV_REST_API_URL), per-asset keys for simulate.js compatibility
...
Respond ONLY with JSON: {"agree":true,"adjusted_confidence":68,"risk_flags":["specific risk 1","specific risk 2"],"contrarian":"strongest counter-argument","final_recommendation":"CONFIRM"}`;
  ...
  try {
    ...
    if (redis) {
      ...
      // FIXED: Use per-asset key to match simulate.js expectations
      const assetKey = asset || 'BTC';
      await redis.lpush(`signals:${assetKey}`, signalRecord);
