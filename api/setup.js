...
// FIXED: Removed window.ethereum (server-side), correct chainId, real endpoints
...
export default async function handler(req, res) {
  ...
  try {
      ...
      return res.json({
        ...
        guide: {
          steps: [
            {
              ...
              // FIXED: Correct chainId hex values
              // 138565 = 0x21D45 (testnet), 286623 = 0x45F5F (mainnet)
          ...
          constraints: {
            ...
            signaturePrefix: '0x02' // FIXED: addAPIKey uses 0x02 prefix per SoDEX docs
          }
        }
      });
    }
    ...
    if (action === 'account-id') {
      ...
      try {
        ...
        if (r.ok) {
          ...
          // FIXED: SoDEX returns 'aid' field for account ID
    ...
    if (action === 'prepare-add-key' && req.method === 'POST') {
      ...
      // FIXED: Correct EIP-712 for addAPIKey (universal domain per SoDEX docs)
      const domain = {
        name: "universal",
        version: "1",
        // FIXED: domain.chainId must match X-API-Chain header
      ...
      const message = {
        chainID: isTestnet ? 138565 : 286623, // message.chainID selects network
        nonce: nonce,
        accountID: parseInt(accountID),
        name: keyName,
        keyType: 1, // typed data uses keyType
    ...
    if (action === 'submit-add-key' && req.method === 'POST') {
      ...
      // FIXED: Use correct endpoint /trade/addAPIKey (not /accounts/add-api-key)
      const r = await fetch(`${base}/trade/addAPIKey`, {
        ...
        body: JSON.stringify({
          accountID: parseInt(accountID),
          name: keyName,
          type: 1, // JSON payload uses 'type', typed data uses 'keyType'
          publicKey: keyPublicKey,
          expiresAt: 0,
    ...
    if (action === 'list-keys') {
      ...
      // FIXED: No window reference - this is server-side only
