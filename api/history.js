...
// FIXED: Redis env var (KV_REST_API_URL), reads per-asset keys
...
// FIXED: Use KV_REST_API_URL (https://) not KV_URL (redis://)
function getRedis() {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'get' || !action) {
      ...
      if (redis) {
        try {
          // FIXED: Read per-asset key if asset specified, else global
    ...
    if (action === 'backtest') {
      ...
      // Delegate to simulate.js logic
      return res.status(200).json({
        ok: false,
        error: 'Use /api/simulate for backtesting. This endpoint is deprecated.',
