...
// FIXED: Real endpoints, correct EIP-712 schema per SoDEX docs
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'account-state') {
      ...
      // FIXED: Use /accounts/{address}/state (confirmed real endpoint)
    ...
    if (action === 'prepare-order' && req.method === 'POST') {
      ...
      // FIXED: Correct EIP-712 Domain for trading actions per SoDEX docs
      const domain = {
        name: "spot",
        version: "1",
      ...
      // FIXED: Correct EIP-712 types for ExchangeAction
      const types = {
        EIP712Domain: [
      ...
      // FIXED: Correct payload structure per SoDEX docs
      // payloadHash = keccak256(compact JSON of {type, params})
      const nonce = Date.now();

      const orderParams = {
        accountID: parseInt(accountID),
        symbolID: symbol, // SoDEX resolves this
        orders: [{
          ...
          type: 2, // market = 2
          timeInForce: 3, // GTC
          price: String(price), // DecimalString as string
          quantity: String(quantity), // DecimalString as string
          reduceOnly: false,
          positionSide: 1
        }]
      };
      ...
      // Note: Client must compute keccak256(JSON.stringify(payload)) for payloadHash
      // We return the payload so client can hash it with ethers.js
      ...
      return res.json({
        ok: true,
        payload,
        payloadStr, // Client hashes this
        domain,
        types,
        primaryType: 'ExchangeAction',
        message: {
          payloadHash: "0x", // Placeholder - client computes with ethers.keccak256
          nonce
        },
        eip712: true,
    ...
    if (action === 'submit-order' && req.method === 'POST') {
      ...
      // FIXED: Use correct endpoint /trade/orders/batch with proper headers
      const r = await fetch(`${base}/trade/orders/batch`, {
        ...
        body: JSON.stringify(params), // params only, not wrapped
