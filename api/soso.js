// SoSoValue API — Optimized with chunked fetching, multi-source ETF fallback
const BASE = 'https://openapi.sosovalue.com/openapi/v1';

// Farside Investors free API for ETF flows (no key needed)
async function fetchFarsideETF() {
  try {
    const r = await fetch('https://farside.co.uk/api/etf/flows/latest', {
    ...
    const flows = [];
    for (const [ticker, flow] of Object.entries(data.data)) {
      if (nameMap[ticker]) {
        flows.push({ ticker, name: nameMap[ticker], netInflow: parseFloat(flow || 0) });
      }
    }
    return flows.length > 0 ? flows : null;
  } catch (e) {
    console.error('Farside fallback:', e.message);
    return null;
  }
}
  ...
  try {
    if (type === 'etf-flows') {
      ...
      for (let i = 0; i < tickers.length; i += chunkSize) {
        ...
        const results = await Promise.all(chunk.map(async ({ t, n }) => {
      ...
      if ((etfs.filter(e => Math.abs(e.netInflow) > 0).length < 5 || Math.abs(sumFlows) < 1000) && Math.abs(totalNet) > 0) {
        const farsideFlows = await fetchFarsideETF();
        if (farsideFlows && farsideFlows.length >= 5) {
          etfs = farsideFlows;
          totalNet = etfs.reduce((a, b) => a + b.netInflow, 0);
          source = 'Farside';
        } else {
          const shares = {
    ...
    if (type === 'sector' || type === 'ssi') {
      ...
      for (let i = 0; i < tickerList.length; i += chunkSize) {
        ...
        ssiData.push(...results.filter(Boolean));
        if (i + chunkSize < tickerList.length) await new Promise(r => setTimeout(r, 100));
      }

      console.log('SSI success:', ssiData.length, 'out of', tickerList.length);
