...
// FIXED: Rate-limited chunked requests, CoinGlass ETF fallback
const BASE = 'https://openapi.sosovalue.com/openapi/v1';

// CoinGlass free API fallback for ETF flows
async function fetchCoinGlassETF() {
  try {
    const r = await fetch('https://api.coinglass.com/api/v1/etf/flow', {
    ...
    for (const item of (data?.data || [])) {
      ...
      if (nameMap[ticker]) {
        flows.push({
          ticker,
          name: nameMap[ticker],
          netInflow: parseFloat(item.netInflow || item.flow || 0)
        });
      }
    }
    return flows.length > 0 ? flows : null;
  } catch (e) {
    console.error('CoinGlass fallback:', e.message);
    return null;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'etf-flows') {
      ...
      // FIXED: Chunked requests with 100ms stagger to respect 20 req/min
      const chunkSize = 5;
      const etfs = [];
      for (let i = 0; i < tickers.length; i += chunkSize) {
        const chunk = tickers.slice(i, i + chunkSize);
        const chunkResults = await Promise.all(chunk.map(async ({ t, n }) => {
        ...
        etfs.push(...chunkResults);
        if (i + chunkSize < tickers.length) await new Promise(r => setTimeout(r, 100));
      }

      const sumFlows = etfs.reduce((a, b) => a + b.netInflow, 0);

      // Fallback to CoinGlass if SoSoValue data is sparse
      if ((etfs.filter(e => Math.abs(e.netInflow) > 0).length < 5 || Math.abs(sumFlows) < 1000) && Math.abs(totalNet) > 0) {
        const coinGlassFlows = await fetchCoinGlassETF();
        if (coinGlassFlows && coinGlassFlows.length >= 5) {
          const cgTotal = coinGlassFlows.reduce((a, b) => a + b.netInflow, 0);
          res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate');
          return res.json({
            ok: true,
            data: { etfs: coinGlassFlows.slice(0, 12), totalNet: cgTotal, totalAssets: 0, date: new Date().toISOString().split('T')[0] },
            source: 'CoinGlass',
            sosoUsed: false
          });
        }
    ...
    if (type === 'sector' || type === 'ssi') {
      ...
      // FIXED: Chunked requests with 100ms stagger
      const chunkSize = 5;
      const ssiData = [];
      for (let i = 0; i < tickerList.length; i += chunkSize) {
        ...
        ssiData.push(...chunkResults.filter(Boolean));
        if (i + chunkSize < tickerList.length) await new Promise(r => setTimeout(r, 100));
      }
    ...
    // ── CRYPTO STOCKS (limit to 2 for display) ─────────────────────
