...
async function getJson(url, headers) {
  ...
  if (!r.ok) {
    let bodyText = '';
    try { bodyText = (await r.text()).slice(0, 300); } catch (e) { /* ignore */ }
    throw new Error(`HTTP ${r.status} for ${url} — ${bodyText}`);
  }
...
async function fetchEtfFlows(headers) {
  // CONFIRMED via live error: "缺少必须的[String]类型参数" (missing required
  // [String] parameter) — /etfs requires symbol and country_code, it is not
  // a parameterless list endpoint.
  const list = await getJson(`${SOSO_BASE}/etfs?symbol=BTC&country_code=US`, headers);
  ...
  for (const e of targets) {
    try {
      ...
      console.error(`[soso] etf-flows: ${e.ticker} snapshot failed — ${e2.message}`);
    }
    await sleep(120);
  }
  ...
  for (const t of targets) {
    try {
      ...
      out.push({
        ...
        ticker: t.ticker || '',
        btc: latest ? (parseFloat(latest.btc_holding) || 0) : 0
      });
    } catch (e) {
      console.error(`[soso] treasury: ${t.ticker} purchase-history failed — ${e.message}`);
...
async function fetchCryptoStocks(headers) {
  ...
  for (const s of targets) {
    try {
      ...
      console.error(`[soso] crypto-stocks: ${s.ticker} snapshot failed — ${e.message}`);
    }
    await sleep(120);
  }
  return out.length ? out : null;
