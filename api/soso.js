// SoSoValue API — Optimized with parallel fetching, full SSI support, live data
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'etf-flows') {
      ...
      // Only use fallback if API data is completely empty
      if ((etfs.filter(e => Math.abs(e.netInflow) > 0).length < 3 || Math.abs(sumFlows) < 1000) && Math.abs(totalNet) > 0) {
        const shares = {
      ...
      // SHORT cache: 60s for ETF (not 5 min)
    ...
    if (type === 'sector' || type === 'ssi') {
      ...
      if (ssiData.length > 0) {
        res.setHeader('Cache-Control', 's-maxage=60, stale-while-revalidate');
        return res.json({ ok: true, data: ssiData, source: 'SoSoValue', successCount: ssiData.length, updatedAt: Date.now() });
      }
    ...
    if (type === 'prices') {
      ...
      return res.json({ ok: true, data: priceMap, source: sosoSuccess ? 'SoSoValue' : 'Static', sosoUsed: sosoSuccess, updatedAt: Date.now() });
    }
    ...
    if (type === 'treasury') {
      ...
      return res.json({ ok: true, data: companies, source: 'SoSoValue', sosoUsed: true, updatedAt: Date.now() });
    }
    ...
    if (type === 'crypto-stocks') {
      ...
      res.setHeader('Cache-Control', 's-maxage=60, stale-while-revalidate');
      return res.json({ ok: true, data: result, source: result.some(r=>r.source==='SoSoValue') ? 'SoSoValue' : 'Yahoo', updatedAt: Date.now() });
    }
